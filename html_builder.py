...
def _inject_simple(soup, fields):
    ...
    for td in [pre_td, post_td]:
        ...
        for ul in td.find_all('ul'):
            if not ul.get('style'):
                ul['style'] = "margin: 0 0 16px 0; padding-left: 0; list-style-position: inside;"
...
def apply_email_fixes(html: str) -> str:
    ...
    for ul in soup.find_all('ul'):
        ...
        if not ul_style:
            ...
            if 'list-style' not in ul_style.lower():
                ul_style = ul_style.rstrip('; ') + '; list-style-position: inside;'
            ul['style'] = ul_style
    ...
    def _strip_responsive_link_fz(m):
        ...
        p_html = re.sub(
            r'(<(?:a|span)\b[^>]*\bstyle="[^"]*?)\bfont-size:\s*\d+px\s*;?\s*',
            r'\1',
            p_html,
            flags=re.IGNORECASE,
        )
